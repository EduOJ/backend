...
func RegisterDefaultTranslations(v *validator.Validate, trans ut.Translator) (err error) {
	...
	translations := []struct {
		...
		},
		{
			...
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				...
				case reflect.String:
					...
					t, err = ut.T("len-string", FieldTranslations[fe.Field()], c)
				...
				case reflect.Slice, reflect.Map, reflect.Array:
					...
					t, err = ut.T("len-items", FieldTranslations[fe.Field()], c)

				default:
					t, err = ut.T("len-number", FieldTranslations[fe.Field()], ut.FmtNumber(f64, digits))
				}
		...
		},
		{
			...
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				...
				case reflect.String:
					...
					t, err = ut.T("min-string", FieldTranslations[fe.Field()], c)
				...
				case reflect.Slice, reflect.Map, reflect.Array:
					...
					t, err = ut.T("min-items", FieldTranslations[fe.Field()], c)

				default:
					t, err = ut.T("min-number", FieldTranslations[fe.Field()], ut.FmtNumber(f64, digits))
				}
		...
		},
		{
			...
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				...
				case reflect.String:
					...
					t, err = ut.T("max-string", FieldTranslations[fe.Field()], c)
				...
				case reflect.Slice, reflect.Map, reflect.Array:
					...
					t, err = ut.T("max-items", FieldTranslations[fe.Field()], c)

				default:
					t, err = ut.T("max-number", FieldTranslations[fe.Field()], ut.FmtNumber(f64, digits))
				}
		...
		},
		{
			...
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				...
				case reflect.String:
					...
					t, err = ut.T("lt-string", FieldTranslations[fe.Field()], c)
				...
				case reflect.Slice, reflect.Map, reflect.Array:
					...
					t, err = ut.T("lt-items", FieldTranslations[fe.Field()], c)

				case reflect.Struct:
					if fe.Type() != reflect.TypeOf(time.Time{}) {
						err = fmt.Errorf("tag '%s'不能用于struct类型.", fe.Tag())
					} else {
						t, err = ut.T("lt-datetime", FieldTranslations[fe.Field()])
					}
				...
				default:
					...
					t, err = ut.T("lt-number", FieldTranslations[fe.Field()], ut.FmtNumber(f64, digits))
				}
		...
		},
		{
			...
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				...
				case reflect.String:
					...
					t, err = ut.T("lte-string", FieldTranslations[fe.Field()], c)
				...
				case reflect.Slice, reflect.Map, reflect.Array:
					...
					t, err = ut.T("lte-items", FieldTranslations[fe.Field()], c)

				case reflect.Struct:
					if fe.Type() != reflect.TypeOf(time.Time{}) {
						err = fmt.Errorf("tag '%s'不能用于struct类型.", fe.Tag())
					} else {
						t, err = ut.T("lte-datetime", FieldTranslations[fe.Field()])
					}
				...
				default:
					...
					t, err = ut.T("lte-number", FieldTranslations[fe.Field()], ut.FmtNumber(f64, digits))
				}
		...
		},
		{
			...
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				...
				case reflect.String:
					...
					t, err = ut.T("gt-string", FieldTranslations[fe.Field()], c)
				...
				case reflect.Slice, reflect.Map, reflect.Array:
					...
					t, err = ut.T("gt-items", FieldTranslations[fe.Field()], c)
				...
				case reflect.Struct:
					if fe.Type() != reflect.TypeOf(time.Time{}) {
						...
						t, err = ut.T("gt-datetime", FieldTranslations[fe.Field()])
					}
				...
				default:
					...
					t, err = ut.T("gt-number", FieldTranslations[fe.Field()], ut.FmtNumber(f64, digits))
				}
		...
		},
		{
			...
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				...
				case reflect.String:
					...
					t, err = ut.T("gte-string", FieldTranslations[fe.Field()], c)
				...
				case reflect.Slice, reflect.Map, reflect.Array:
					...
					t, err = ut.T("gte-items", FieldTranslations[fe.Field()], c)

				case reflect.Struct:
					if fe.Type() != reflect.TypeOf(time.Time{}) {
						err = fmt.Errorf("tag '%s'不能用于struct类型.", fe.Tag())
					} else {
						t, err = ut.T("gte-datetime", FieldTranslations[fe.Field()])
					}
				...
				default:
					...
					t, err = ut.T("gte-number", FieldTranslations[fe.Field()], ut.FmtNumber(f64, digits))
				}
		...
		},
		{
			...
			override:    false,
			customTransFunc: func(ut ut.Translator, fe validator.FieldError) string {
				s, err := ut.T(fe.Tag(), FieldTranslations[fe.Field()], fe.Param())
				if err != nil {
					log.Printf("警告: 翻译字段错误: %#v", fe)
					return fe.(error).Error()
				}
...
func translateFunc(ut ut.Translator, fe validator.FieldError) string {
	...
	t, err := ut.T(fe.Tag(), FieldTranslations[fe.Field()])
	if err != nil {
		log.Printf("警告: 翻译字段错误: %#v", fe)
		return fe.(error).Error()
	}
